...
class chessConstructor {
	...
	constructor(sSelector){
		...
		this.selectedFigureColor = null;
		this.selectedFigureCurrPos = null;
		this.selectedFigureStep = 0;
	...
	placeFiguresOnTable() {
		for(let key in this.figuresOnTable) {
			...
			div.id = this.figuresOnTable[key].type + "_" + this.figuresOnTable[key].color + "_" + this.figuresOnTable[key].startPos;
	...
	clickOnCell(e) {
		if (!this.chessFigureIsSelected) {
			if (e.target.classList.contains('chess_figure') && this.figuresOnTable[e.target.id].color === this.whoseTurn) {
				...
				this.figuresOnTable[e.target.id].selected = true;
				this.selectedFigure = e.target.id;
				this.selectedFigureStep = this.figuresOnTable[this.selectedFigure].step;
				this.selectedFigureType = this.figuresOnTable[this.selectedFigure].type;
				this.selectedFigureColor = this.figuresOnTable[this.selectedFigure].color;
				this.selectedFigureCurrPos = this.figuresOnTable[this.selectedFigure].currentPos;
			}
		} else {
			...
			this.figuresOnTable[this.selectedFigure].selected = false;
			this.selectedFigure = null;
			this.selectedFigureStep = 0;
			this.selectedFigureType = null;
			this.selectedFigureColor = null;
	...
	ifCanAttack(e) {
		return this.selectedFigureColor !== this.figuresOnTable[e.target.id].color;
	}

	canFigureMoveThere(e) {
		let usingType = this.howIWantToMove(e);
		if (this.comparingTypeOfMove(usingType, this.figuresOnTable[this.selectedFigure].movementType) && !this.isThereAnyFigureOnWay(e, usingType)){
			return true;
		} else {
			return false;
		}
	...
	howIWantToMove(e){
		let letterFrom = this.selectedFigureCurrPos.charAt(0),
			letterTo = this.attacking ? e.target.parentElement.id.charAt(0) : e.target.id.charAt(0),
			numberFrom = parseInt(this.selectedFigureCurrPos.charAt(1)),
		...
		console.log(letterFrom, numberFrom, letterTo, numberTo, this.selectedFigure, this.attacking, e.target.id, e.target.parentElement.id)

		if ((this.selectedFigureType === 'pawn') && onLetters && ( ( (numberFrom < numberTo) && (numberFrom >= numberTo - this.selectedFigureStep) && (this.selectedFigureColor === 'white')) || ((numberFrom > numberTo) && (numberFrom <= parseInt(numberTo) + this.selectedFigureStep) && (this.selectedFigureColor === 'black')))){
			usingType = 'pawn';
	...
	comparingTypeOfMove(usingType, needsType) {
		if (needsType.indexOf('_') === -1) {
			if (usingType === needsType && this.stepWantsToMove <= this.selectedFigureStep) return true;
		} else {
			let typesArray = needsType.split('_');
			for (let i = 0; i < typesArray.length; i++) {
				if (usingType === typesArray[i] && this.stepWantsToMove <= this.selectedFigureStep) return true;
			}
		}
		return false;
	}
	...
	isThereAnyFigureOnWay(e, usingType) {
		...
		console.log(letterFrom, numberFrom, letterTo, numberTo, e.target.id, this.selectedFigureCurrPos)

		if (usingType === 'horse') return false;

		if (letterFrom === letterTo) {
			if (numberFrom < numberTo) {
				for (let i = numberFrom + 1; i < numberTo; i++) {
	...
	attack(e){
		...
		selectedCell.appendChild(document.getElementById(this.figuresOnTable[this.selectedFigure].name));
		this.figuresOnTable[this.selectedFigure].currentPos = selectedCell.id;
		if (this.selectedFigureType === 'pawn') {
			this.figuresOnTable[this.selectedFigure].step = 1;
		}
		return true;
	}

	moveFigure(e) {
		e.target.appendChild(document.getElementById(this.figuresOnTable[this.selectedFigure].name));
		this.figuresOnTable[this.selectedFigure].currentPos = e.target.id;
		if (this.selectedFigureType === 'pawn') {
			this.figuresOnTable[this.selectedFigure].step = 1;
		}
		return true;
